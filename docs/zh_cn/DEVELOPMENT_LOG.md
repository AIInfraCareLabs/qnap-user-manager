...
[English](../../DEVELOPMENT_LOG.md) | [中文文档目录](README.md) | [中文 README](../../README-zh_cn.md)
...
4. 优先候选：CLI 两步验证交互和 Windows 实测（Linux Python 3.11/3.13 已由 CI 验证）、创建资源多步骤补偿、细粒度文件 ACL 与应用权限、配额协议验证；AD/LDAP 需独立适配，不混入本地用户假设。
...
默认根目录文档与 docs 已切换为英文。中文入口为 [README-zh_cn.md](../../README-zh_cn.md)，中文指南位于 docs/zh_cn/ 并与英文互链。源码包收录两种语言。此次文档改动不覆盖已发布的 0.4.0，也不创建新 Release；下次版本发布时 PyPI README 将使用英文。后续开发同步两种语言，继续使用 puluto 与 GitHub 隐私邮箱提交。

双语整理验证：20 份 Markdown 的默认英文、相对链接与中文 README 路由检查通过；源码包包含两种语言，wheel 元数据读取英文 README。68 项离线测试、Twine 与发行内容检查均通过。
